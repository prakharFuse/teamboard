...
- `is_active` exists and `GET /api/members` filters on it (`is_active = 1`). `DELETE /api/members/:id` soft-deletes: it sets `is_active = 0` and prefixes `email` with `deactivated-` (`members.ts:106-121`) rather than deleting the row — see [[gotchas]] for the email-prefix idempotency detail and the SSO-revocation rationale.
