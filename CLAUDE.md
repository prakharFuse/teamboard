...
- **DELETE**: Soft deletes only — sets `is_active = 0` and prefixes the email with `"deactivated-"` so the Okta nightly sync revokes SSO access. Rows are never physically removed.
