...
- **DELETE**: Soft-deletes: sets `is_active = 0` and prefixes email with `"deactivated-"` for the nightly Okta sync. Records are never hard-deleted.
...
| DELETE | `/api/members/:id` | Deactivate member (soft-delete): sets is_active = 0 and prefixes email with "deactivated-" for Okta SSO revocation |
