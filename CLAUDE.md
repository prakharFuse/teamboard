...
- **DELETE**: Soft deletes — sets is_active = 0 and prefixes email with deactivated- for Okta SSO revocation. Records are retained for HR compliance (7-year retention policy).
...
| DELETE | `/api/members/:id` | Soft delete (deactivate) member — sets is_active = 0 and prefixes email |
