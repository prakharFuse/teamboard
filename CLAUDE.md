...
- **DELETE**: Soft deletes: sets `is_active = 0` and prefixes `email` with `deactivated-` for Okta SSO nightly revocation. Records are never hard-deleted (7-year HR retention policy).
...
| DELETE | `/api/members/:id` | Soft-delete member: sets is_active = 0 and prefixes email with `deactivated-` (triggers Okta SSO revocation on nightly sync) |
