...
- **DELETE**: Soft deletes — sets `is_active = 0` and prefixes email with `deactivated-` (for Okta SSO nightly sync revocation). Records are never hard-deleted per the 7-year HR retention policy.
...
| DELETE | `/api/members/:id` | Soft delete (deactivate) member — sets is_active=0, prefixes email with deactivated- |
