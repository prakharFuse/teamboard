...
- **DELETE**: Soft deletes — sets `is_active = 0` and prefixes the email with `deactivated-` (e.g. `sarah@company.com` → `deactivated-sarah@company.com`) for Okta SSO revocation. The row is preserved for HR compliance/audit purposes. Returns `400` if the member is already inactive.
