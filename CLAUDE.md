...
- **DELETE**: Soft-deactivates the member (sets `is_active = 0`, prefixes email with `deactivated-` for Okta SSO revocation). All other columns (name, role, department, start_date) are preserved. Idempotent — calling DELETE on an already-inactive member returns `{ success: true }` without re-applying changes.
...
| DELETE | `/api/members/:id` | Soft-deactivate: sets `is_active = 0`, prefixes email with `deactivated-` (preserves all other columns); idempotent |
