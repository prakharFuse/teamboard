...
import { DEPARTMENT_CODES, isValidDeptCode, deptName } from '../departments.js';
...
router.post('/', (req: Request, res: Response): void => {
  ...
  if (!isValidDeptCode(department)) {
    res.status(400).json({ error: `Invalid department code. Must be one of: ${DEPARTMENT_CODES.join(', ')}` });
    return;
  }
  const db = getDb();
  try {
...
router.get('/export', (req: Request, res: Response): void => {
  ...
  const csv = [header, ...rows.map(r =>
    `${r.id},${r.name},${r.email},${r.role},${deptName(r.department) ?? r.department},${r.department},${r.start_date},${r.is_active}`
  )].join('\n');
