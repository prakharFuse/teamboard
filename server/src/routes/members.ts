...
import { isValidDepartment, validDepartmentsList } from '../departments.js';
...
router.post('/', (req: Request, res: Response): void => {
  ...
  if (!isValidDepartment(department)) {
    res.status(400).json({ error: `Invalid department '${department}'. Allowed values: ${validDepartmentsList()}` });
    return;
  }
  const db = getDb();
  try {
...
router.patch('/:id', (req: Request, res: Response): void => {
  ...
  if (department !== undefined && department !== null && !isValidDepartment(department)) {
    res.status(400).json({ error: `Invalid department '${department}'. Allowed values: ${validDepartmentsList()}` });
    return;
  }
  db.prepare(
