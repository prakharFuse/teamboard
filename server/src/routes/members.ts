...
import { isValidDeptCode, getAllowedCodes } from '../departments.js';
...
router.post('/', (req: Request, res: Response): void => {
  ...
  if (!isValidDeptCode(department)) {
    res.status(400).json({ error: `Invalid department code "${department}". Allowed codes: ${getAllowedCodes().join(', ')}` });
    return;
  }
  const db = getDb();
  try {
