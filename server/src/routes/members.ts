...
import { isValidDepartmentCode, departmentName, DEPARTMENT_CODES } from '../departments.js';
...
router.post('/', (req: Request, res: Response): void => {
  ...
  if (!isValidDepartmentCode(department)) {
    res.status(400).json({ error: `Invalid department. Allowed codes: ${DEPARTMENT_CODES.join(', ')}` });
    return;
  }
  const db = getDb();
  try {
...
router.get('/export', (req: Request, res: Response): void => {
  ...
  const header = 'id,name,email,role,department,dept_code,start_date,is_active';
  const csv = [header, ...rows.map(r =>
    `${r.id},${r.name},${r.email},${r.role},${departmentName(r.department) ?? r.department},${r.department},${r.start_date},${r.is_active}`
  )].join('\n');
