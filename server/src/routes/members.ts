...
router.delete('/:id', (req: Request, res: Response): void => {
  ...
  // TM-106 root cause fix: this used to be a hard `DELETE FROM members`,
  // which removes the row outright so `is_active` never goes to 0 and the
  // email is never prefixed with 'deactivated-'. The external Okta sync
  // keys off exactly that signal (is_active=0 + 'deactivated-' email
  // prefix) to deprovision SSO; a physically absent row produces neither,
  // so a departed employee's SSO access was never revoked (confirmed by
  // SUP-2: 21 daily Okta syncs saw 0 deactivation signals for the affected
  // member). Fixed by soft-deleting in place — is_active=0 plus a
  // 'deactivated-' email prefix — and firing the SSO deprovision dispatch
  // synchronously in this same request so revocation doesn't wait on the
  // nightly sync. No auth/tenant middleware exists anywhere in this
  // service (see index.ts) to mirror for that dispatch call.
  if (member.is_active === 0 || member.email.startsWith('deactivated-')) {
    // Already deactivated: no-op idempotently so a repeat DELETE never
    // double-prefixes the email or re-fires the SSO dispatch.
  ...
  ssoDeprovision.dispatch(member.id);
  res.json({ success: true });
});
