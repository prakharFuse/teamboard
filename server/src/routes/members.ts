...
import { isValidDeptCode, getAllowedDeptCodes } from '../departments.js';
...
router.post('/', (req: Request, res: Response): void => {
  ...
  if (!isValidDeptCode(department)) {
    res.status(400).json({ error: `Invalid department code "${department}". Allowed codes: ${getAllowedDeptCodes().join(', ')}` });
    return;
  }
  const db = getDb();
  try {
