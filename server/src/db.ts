...
export function getDb(): DatabaseSync {
  if (!db) {
    ...
    // TODO: Replace placeholder dept_codes ('Engineering', 'HR') with the real BambooHR
    // codes once confirmed via the People Ops quarterly CSV attachment (see TEAM-4).
    // Only runs when stale rows are actually present so it does not execute on every startup.
    const staleRows = db.prepare(
      "SELECT COUNT(*) as count FROM members WHERE department IN ('Eng', 'Human Resources')"
    ).get() as { count: number };
    if (staleRows.count > 0) {
      db.exec(`
        UPDATE members SET department = 'Engineering' WHERE department = 'Eng';
        UPDATE members SET department = 'HR' WHERE department = 'Human Resources';
      `);
    }
